#!/usr/bin/env bash
# Usage: run_epoch.sh <transformers-version> <tag> [torch-version] [cu-index]   e.g. run_epoch.sh 4.53.3 tf453 2.3.1 cu121
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"; TV=$1; TAG=$2; TORCHV=${3:-2.6.0}; CU=${4:-cu124}; V="$HERE/.venv_$TAG"; L="$HERE/run.log"
...
echo "== $(date -Is) EPOCH $TAG transformers==$TV torch==$TORCHV/$CU start" | tee -a "$L"
[ -x "$V/bin/python" ] || { uv venv -q "$V" --python 3.11 && uv pip install -q --python "$V/bin/python" pip "setuptools<81" wheel \
  && uv pip install -q --python "$V/bin/python" "torch==$TORCHV" --index-url https://download.pytorch.org/whl/$CU \
